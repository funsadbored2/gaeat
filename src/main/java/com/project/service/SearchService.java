...
public class SearchService {
	...
	public List<Integer> miunsFiltering(List<String> minusWord) {
		...
		String[] m_wno = null;
		if (minusWordNo != null) {
			m_wno = minusWordNo.split(",");
		} else {
			return searchdao.getAllrecipeNo();
		}
	...
	public List<Integer> miunsFilteringT(keywordVo kvo) {
		if (kvo.getCheck_login() == 1) { // 로그인 사용자일경우 추가한다.
			...
			if (dm != null) {
				Pattern pattern = Pattern.compile(",");
				List<Integer> dmlist = pattern.splitAsStream(dm).map(Integer::valueOf).collect(Collectors.toList());
				// 차집합
	...
	public List<Integer> SplitAndIntersection(String str1, String str2) {
		...
		if (str1 != null) {
			list1 = pattern.splitAsStream(str1).map(Integer::valueOf).collect(Collectors.toList());
		}
		if (str2 != null) {
			list2 = pattern.splitAsStream(str2).map(Integer::valueOf).collect(Collectors.toList());
		}
